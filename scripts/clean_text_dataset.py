...
def clean_text(text_list,system_role,model_id,seed=42,n_gpu=4,batch_size=128):
    ...
    sampling_params = SamplingParams(temperature=0.7, top_p=.8,repetition_penalty=1.05, max_tokens=45000,seed=seed)
...
def main(system_role,model_id,path='../data/impresso'):
    ...
    generated_dataset=clean_text(df['content'].fillna('Null').tolist(),system_role,model_id,n_gpu=n_gpu,batch_size=batch_size*n_gpu)
    generated_dataset.save_to_disk(f'{path}/impresso160k_clean_seq45k')
